...
*/
const myFavoriteAnimes = ["one piece","naruto","attack on titan"]

const sentence = `Minhas 3 séries favoritas são: ${myFavoriteAnimes[myFavoriteAnimes.indexOf('one piece')]}, ${myFavoriteAnimes[myFavoriteAnimes.indexOf('naruto')]} e ${myFavoriteAnimes[myFavoriteAnimes.indexOf('attack on titan')]}.`
console.log(myFavoriteAnimes)
...
const isRandomTVShowIncluded=Boolean(randomTVShow.includes('Watchmen'))
...
*/
const typesSentence = `O tipo de dado que a const "isRandomTVShowIncluded" armazena é: ${typeof(isRandomTVShowIncluded)}.`
console.log(typesSentence)
...
/*
  ...
  caracteres? BOOLEAN."
*/
let phrase = `A string que a "typeSentence" armazena tem mais de 39
caracteres? ${String(typesSentence.length===39)}.`
console.log(phrase)

let test = `${String(typesSentence.length===39).slice(0,1).toUpperCase()}${String(typesSentence.length===39).slice(1)}`
console.log(test)
/*
...
console.log(`Todos ${falsyValues.length} itens do array "falsyValues" são falsy. Inclusive o  
${falsyValues[falsyValues.indexOf(false)]}.`)
...
const isNotString = typeof(randomTVShow)===String
...
const eightExist = evenNumbers.indexOf(8)!= -1
console.log(eightExist)